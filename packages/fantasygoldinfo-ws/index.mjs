...
import {Transaction, Address} from 'fantasygoldinfo-lib'
import Service from 'fantasygoldinfo-node/lib/services/base'

export default class FantasyGoldinfoWebsocketService extends Service {
  constructor(options) {
    super(options)
  ...
  async start() {
    this._bus = this.node.openBus({remoteAddress: 'localhost-fantasygoldinfo-ws'})
  ...
  async _transformTransaction(transaction) {
    ...
    let fgc20TokenTransfers = await this.node.getFGC20TokenTransfers(transaction)
    transformed.fgc20TokenTransfers = fgc20TokenTransfers.map(({token, from, to, amount}) => ({
      token: {
    ...
    let fgc721TokenTransfers = await this.node.getFGC721TokenTransfers(transaction)
    transformed.fgc721TokenTransfers = fgc721TokenTransfers.map(({token, from, to, tokenId}) => ({
      token: {
        address: token.address.toString('hex'),
        name: token.name,
        symbol: token.symbol,
