import {Address, Script, Solidity} from 'fantasygoldinfo-lib'
...
export default class AddressService extends Service {
  ...
  async getAddressBalanceHistory(
    ...
    addresses = parseAddresses(addresses)
    let sort = reversed ? {'block.height': -1, index: -1} : {'block.height': 1, index: 1}
    let count = await FantasyGoldBalanceChanges.countDocuments({address: {$in: addresses}})
    let list = await FantasyGoldBalanceChanges.aggregate([
    ...
    let [initialBalance] = await FantasyGoldBalanceChanges.aggregate([
      {
        $match: {
