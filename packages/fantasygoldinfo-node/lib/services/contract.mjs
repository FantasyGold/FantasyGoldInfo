...
import {Address, Solidity} from 'fantasygoldinfo-lib'
import Contract from '../models/contract'
import Transaction from '../models/transaction'
import TransactionOutput from '../models/transaction-output'
import FantasyGoldBalanceChanges from '../models/fantasygold-balance-changes'
import FGC20TokenBalance from '../models/fgc20-token-balance'
import Service from './base'
import {toBigInt} from '../utils'

const totalSupplyABI = Solidity.fgc20ABIs.find(abi => abi.name === 'totalSupply')
const balanceOfABI = Solidity.fgc20ABIs.find(abi => abi.name === 'balanceOf')
const ownerOfABI = Solidity.fgc721ABIs.find(abi => abi.name === 'ownerOf')
...
export default class ContractService extends Service {
  ...
  get APIMethods() {
    return {
      ...
      getFGC20TokenTransfers: this.getFGC20TokenTransfers.bind(this),
      getFGC721TokenTransfers: this.getFGC721TokenTransfers.bind(this),
      getAddressFGC20TokenBalanceHistory: this.getAddressFGC20TokenBalanceHistory.bind(this),
      listFGC20Tokens: this.listFGC20Tokens.bind(this),
      getAllFGC20TokenBalances: this.getAllFGC20TokenBalances.bind(this),
      searchFGC20Token: this.searchFGC20Token.bind(this),
      getFGC20TokenHolders: this.getFGC20TokenHolders.bind(this),
      getFGC20TokenRichList: this.getFGC20TokenRichList.bind(this)
    }
  }
  ...
  async getContract(address) {
    ...
    return contract && {
      ...
      type: contract.type,
      ...contract.fgc20
        ? {fgc20: parseFGC20(contract.fgc20)}
        : {},
      ...contract.fgc721
        ? {fgc721: parseFGC721(contract.fgc721)}
        : {}
    }
  }
  ...
  async getContractSummary(address) {
    ...
    let [balanceChangesResult] = await FantasyGoldBalanceChanges.aggregate([
      {
        $match: {
  ...
  async getFGC20TokenTransfers(transaction) {
    let list = []
    let tokenCache = {}
    let addressCache = {}
    for (let receipt of transaction.receipts) {
      for (let {address, topics, data} of receipt.logs) {
        ...
        if (addressString in tokenCache) {
          ...
          let contract = await Contract.findOne({address, type: 'fgc20'})
          if (!contract) {
            continue
          }
          tokenCache[addressString] = token = {address, ...parseFGC20(contract.fgc20)}
        }
  ...
  async getFGC721TokenTransfers(transaction) {
    let list = []
    let tokenCache = {}
    let addressCache = {}
    for (let receipt of transaction.receipts) {
      for (let {address, topics, data} of receipt.logs) {
        ...
        if (addressString in tokenCache) {
          ...
          let contract = await Contract.findOne({address, type: 'fgc721'})
          if (!contract) {
            continue
          }
          tokenCache[addressString] = token = {address, ...parseFGC721(contract.fgc721)}
        }
        let fromString = topics[1].slice(12).toString('hex')
        let toString = topics[2].slice(12).toString('hex')
        let from
  ...
  async getAddressFGC20TokenBalanceHistory(addresses, tokens, {pageIndex = 0, pageSize = 100, reversed = true} = {}) {
                ...
                logs: {
                  $push: {
                    token: {
                      ...
                      name: '$token.fgc20.name',
                      symbol: '$token.fgc20.symbol',
                      decimals: '$token.fgc20.decimals',
                      totalSupply: '$token.fgc20.totalSupply',
                      version: '$token.fgc20.version'
                    },
                    topics: '$log.topics',
                    data: '$log.data'
                  }
    ...
    let tokenBalances = {}
    for (let {contract, balance} of await FGC20TokenBalance.find({address: {$in: hexAddresses.map(address => address.slice(24))}})) {
  ...
  async listFGC20Tokens({pageIndex = 0, pageSize = 100}) {
    let [{count, list}] = await FGC20TokenBalance.aggregate([
      {
        $match: {
    ...
    return {
      totalCount: count.length && count[0].count,
      tokens: list.map(({address, fgc20, holders}) => ({
  ...
  async getAllFGC20TokenBalances(addresses) {
    if (!Array.isArray(addresses)) {
      addresses = [addresses]
    }
    let hexAddresses = addresses
    ...
    let list = await FGC20TokenBalance.aggregate([
    ...
    return list.map(({address, fgc20, balances}) => {
      let sum = 0n
  ...
  async searchFGC20Token(name, {limit = 1} = {}) {
    ...
    return result.map(({address, fgc20, holders}) => ({
      address: Buffer.from(address, 'hex'),
      ...parseFGC20(fgc20),
      holders
    }))
  ...
  async getFGC20TokenHolders(token) {
    return await FGC20TokenBalance.countDocuments({
      contract: token,
      balance: {$ne: 0n}
    })
  }

  async getFGC20TokenRichList(token, {pageIndex = 0, pageSize = 100} = {}) {
    let totalCount = await FGC20TokenBalance.countDocuments({
      contract: token,
      balance: {$ne: 0n}
    })
    let list = await FGC20TokenBalance.collection
      .find(
  ...
  async onReorg(height) {
    ...
    await FGC20TokenBalance.deleteMany({contract: {$in: contracts}})
  ...
  async _syncContracts() {
    ...
    await FGC20TokenBalance.deleteMany({contract: {$in: contractsToRemove}})
  ...
  async _createContract(address, {transaction, block, owner, code} = {}) {
    ...
    if (isFGC721(code)) {
      ...
      let [nameResult, symbolResult, totalSupplyResult] = await this._batchCallMethods([
        {address, abi: Solidity.fgc721ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.fgc721ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.fgc721ABIs.find(abi => abi.name === 'totalSupply')}
      ])
      try {
        fgc721Params.name = (await nameResult)[0]
      } catch (err) {}
      try {
        fgc721Params.symbol = (await symbolResult)[0]
      } catch (err) {}
      try {
        fgc721Params.totalSupply = BigInt((await totalSupplyResult)[0].toString())
        contract.type = 'fgc721'
        contract.tags = ['fgc721']
        contract.fgc721 = fgc721Params
      } catch (err) {}
    } else if (isFGC20(code)) {
      let fgc20Params = {}
      let [
        nameResult, symbolResult, decimalsResult, totalSupplyResult, versionResult
      ] = await this._batchCallMethods([
        {address, abi: Solidity.fgc20ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.fgc20ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.fgc20ABIs.find(abi => abi.name === 'decimals')},
        {address, abi: Solidity.fgc20ABIs.find(abi => abi.name === 'totalSupply')},
        {address, abi: Solidity.fgc20ABIs.find(abi => abi.name === 'version')}
      ])
      try {
        fgc20Params.name = (await nameResult)[0]
      } catch (err) {}
      try {
        fgc20Params.symbol = (await symbolResult)[0]
      } catch (err) {}
      try {
        fgc20Params.decimals = (await decimalsResult)[0].toNumber()
      } catch (err) {}
      try {
        fgc20Params.version = (await versionResult)[0]
      } catch (err) {}
      try {
        fgc20Params.totalSupply = BigInt((await totalSupplyResult)[0].toString())
        contract.type = 'fgc20'
        contract.tags = ['fgc20']
        contract.fgc20 = fgc20Params
      } catch (err) {}
    }
  ...
  async _processReceipts(block) {
    ...
    for (let address of totalSupplyChanges) {
      let contract = await Contract.findOne({address, type: {$in: ['fgc20', 'fgc721']}})
      if (contract) {
        try {
          let [totalSupply] = await this._callMethod(Buffer.from(address, 'hex'), totalSupplyABI)
          if (contract.type === 'fgc20') {
            contract.fgc20.totalSupply = BigInt(totalSupply.toString())
          } else {
            contract.fgc721.totalSupply = BigInt(totalSupply.toString())
          }
        } catch (err) {
          continue
        }
  ...
  async _updateBalances(balanceChanges) {
    ...
    if (operations.length) {
      await FGC20TokenBalance.collection.bulkWrite(operations)
    }
  }
