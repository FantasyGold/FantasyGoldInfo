import {Address, Script, Input, Output, Transaction as RawTransaction} from 'fantasygoldinfo-lib'
...
export default class TransactionService extends Service {
  ...
  async getTransaction(id) {
    let [transaction] = await Transaction.aggregate([
      ...
      {
        $lookup: {
          from: 'fantasygoldbalancechanges',
          localField: 'id',
          foreignField: 'id',
          as: 'balanceChanges'
        }
  ...
  async start() {
    ...
    await FantasyGoldBalanceChanges.deleteMany({'block.height': {$gt: this._tip.height}})
  ...
  async onReorg(height) {
    ...
    await FantasyGoldBalanceChanges.bulkWrite([
      {deleteMany: {filter: {id: {$in: outputTransactionIds}}}},
      {
        updateMany: {
  ...
  async _processTransaction(tx, indexInBlock, block) {
    ...
    if (resultTransaction) {
      await FantasyGoldBalanceChanges.updateMany(
        {id: tx.id},
        {
          block: {
            hash: block.hash,
    ...
    if (block.height > 0) {
      ...
      await FantasyGoldBalanceChanges.collection.insertMany(balanceChanges, {ordered: false})
    }
  ...
  async removeReplacedTransactions(tx) {
    ...
    while (replacedTransactions.length !== 0) {
      ...
      await FantasyGoldBalanceChanges.deleteMany({id})
    }
  }
