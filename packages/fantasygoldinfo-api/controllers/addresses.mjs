...
export default class AddressesController {
  ...
  async summary(ctx) {
    ...
    let fgc20TokenBalances = await this.node.getAllFGC20TokenBalances(ctx.state.addresses)
    let balanceRanking
  ...
  async fgc20BalanceHistory(ctx) {
    ...
    let result = await this.node.getAddressFGC20TokenBalanceHistory(
      ctx.state.addresses,
      tokens,
