...
class ContractService extends Service {
  ...
  async start() {
    ...
    this.#FGC20 = this.node.getModel('fgc20')
    this.#FGC20Balance = this.node.getModel('fgc20_balance')
    this.#FGC721 = this.node.getModel('fgc721')
    this.#FGC721Token = this.node.getModel('fgc721_token')
    ...
    for (let transaction of block.transactions) {
      for (let i = 0; i < transaction.outputs.length; ++i) {
        ...
        if (output.scriptPubKey.type === OutputScript.EVM_CONTRACT_CREATE) {
          ...
          if (contract && contract.type === 'fgc20') {
  ...
  async _syncContracts() {
    ...
    if (contractsToRemove.length) {
      await this.#db.query(sql`
        DELETE contract, tag, fgc20, fgc20_balance, fgc721, fgc721_token
        FROM contract
        LEFT JOIN contract_tag tag ON tag.contract_address = contract.address
        LEFT JOIN fgc20 ON fgc20.contract_address = contract.address
        LEFT JOIN fgc20_balance ON fgc20_balance.contract_address = contract.address
        LEFT JOIN fgc721 ON fgc721.contract_address = contract.address
        LEFT JOIN fgc721_token ON fgc721_token.contract_address = contract.address
  ...
  async _createContract(address, vm) {
    ...
    if (isFGC721(code)) {
      ...
      try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'fgc721'})
        await this.#FGC721.create({
      ...
      try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'fgc20'})
        await this.#FGC20.create({
          contractAddress: address,
          name,
          symbol,
          decimals,
    ...
    for (let addressString of totalSupplyChanges) {
      ...
      if (contract) {
        ...
        if (contract.type === 'fgc20') {
          await this.#FGC20.update({totalSupply}, {where: {contractAddress: address}})
        } else {
          await this.#FGC721.update({totalSupply}, {where: {contractAddress: address}})
        }
      }
    }
  }
  ...
  async _updateBalances(balanceChanges) {
    ...
    if (operations.length) {
      await this.#FGC20Balance.bulkCreate(operations, {updateOnDuplicate: ['balance'], validate: false})
    }
  }
  ...
  async _updateTokenHolders(transfers) {
    ...
    await this.#FGC721Token.bulkCreate(operations, {updateOnDuplicate: ['holder'], validate: false})
  }
}
